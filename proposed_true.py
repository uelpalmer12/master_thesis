...
class AutoEncoder(torch.nn.Module):
        def __init__(self, input_dimension, latent_dimension):
            ...
            self.encoder = torch.nn.Sequential(
                ...
                torch.nn.LeakyReLU(negative_slope=0.3),
                torch.nn.Linear(3, latent_dimension)
            )
            ...
            torch.nn.BatchNorm1d(4),
            torch.nn.LeakyReLU(negative_slope=0.3),
            torch.nn.Linear(4,input_dimension)
            )
...
class MultiTaskDKL(gpytorch.models.ExactGP):
        def __init__(self, train_inputs, train_targets, likelihood):
            ...
            base_rbf = gpytorch.kernels.RBFKernel(ard_num_dims=latent_dim, active_dims=[0,1,2])
            base_lin = gpytorch.kernels.LinearKernel(ard_num_dims=latent_dim, active_dims=[0,1,2])


            # TWO independent scales:
            kernel2 = gpytorch.kernels.AdditiveKernel(
                gpytorch.kernels.ScaleKernel(base_rbf),
                gpytorch.kernels.ScaleKernel(base_lin),
            )
            # ONE global scale:
            kernel1 = gpytorch.kernels.ScaleKernel(
                gpytorch.kernels.AdditiveKernel(base_rbf, base_lin)
            )
            ## here we define the continuous as well as embedding task covar module
            self.continuous_covar = gpytorch.kernels.ScaleKernel(gpytorch.kernels.AdditiveKernel(gpytorch.kernels.PeriodicKernel(active_dims=[0,1,2])))
            self.embed_covar = (CosineSimilarityKernel(ard_num_dims = embedding_dim, active_dims = [3,4,5]))
            ...
            self.ae = AutoEncoder(input_dimension= input_dim, latent_dimension= latent_dim)
...
for i in sample_size:
    ...
    mse_mean, width_mean, mean_interval, mspe_width, mspe_percent  = gp_model(X_train=X_train, y_train=y_train, train_task=task_train, test_df=test_df, likelihood=likelihood, training_iter=130, learning_rate=0.06)
...
# after your loop, you have:

df = pd.DataFrame(metrics)
df["model"] = "proposed"   # change per‐script
df.to_csv("metrics_proposed_true.csv", index=False)