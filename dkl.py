...
class MultiTaskDKL(gpytorch.models.ExactGP):
    def __init__(self, train_inputs, train_targets, likelihood):
        ...
        self.continuous_covar = gpytorch.kernels.ScaleKernel(gpytorch.kernels.RBFKernel())
...
def gp_model(X_train, y_train, train_task, test_df, likelihood, training_iter=50, learning_rate = 0.04):
    ...
    for i in range(1,n_tasks+1):
        ...
        with gpytorch.settings.fast_pred_var(False), gpytorch.settings.cholesky_jitter(1e-6):
...
for i in range(0, m):
        print(f"iteration number {i+1} for {50} samples")
    #for j in t:
        ...
        rng = np.random.default_rng(195 * i * 50)
        ...
        for k, s in enumerate(specs, 1):
            df = simulate_process(**s, x_bases=x_bases, T=50, burnin=200, rng=rng)
            df["process"] = k
            frames.append(df)
